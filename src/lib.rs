...
pub fn render_svg(chord_settings: Chord) -> Result<(), std::io::Error> {
    let string_space = 40;
    let margin = 30;
    let width = (4 * margin) + (string_space) * 5;
    ...
    let mut min_fret_marker = "".to_string(); //svg_draw_min_fret(min_fret, &string_space);
                                              // if *lowest_fret > 2 || *lowest_fret > 1 && !show_nut {
    ...
    match Tera::one_off(include_str!("../templates/chord.svg"), &context, true) {
        Ok(result) => {
            println!("{:?}", result);
            let path = "results.svg";
